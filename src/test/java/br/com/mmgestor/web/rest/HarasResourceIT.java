...
public class HarasResourceIT {
     ...
     *
     * @see br.com.mmgestor.repository.search.HarasSearchRepositoryMockConfiguration
     */
    @Autowired
    private HarasSearchRepository mockHarasSearchRepository;
    ...
    public void createHaras() throws Exception {
        ...
        // Validate the Haras in Elasticsearch
        verify(mockHarasSearchRepository, times(1)).save(testHaras);
    }
    ...
    public void createHarasWithExistingId() throws Exception {
        ...
        // Validate the Haras in Elasticsearch
        verify(mockHarasSearchRepository, times(0)).save(haras);
    }
    ...
    public void updateHaras() throws Exception {
        ...
        reset(mockHarasSearchRepository);
    ...
    public void deleteHaras() throws Exception {
        ...
        // Validate the Haras in Elasticsearch
        verify(mockHarasSearchRepository, times(1)).deleteById(haras.getId());
    }

    @Test
    @Transactional
    public void searchHaras() throws Exception {
        // Initialize the database
        harasService.save(haras);
        when(mockHarasSearchRepository.search(queryStringQuery("id:" + haras.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(haras), PageRequest.of(0, 1), 1));
        // Search the haras
        restHarasMockMvc.perform(get("/api/_search/haras?query=id:" + haras.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(haras.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)))
