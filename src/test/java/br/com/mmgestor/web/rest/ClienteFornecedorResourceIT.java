...
public class ClienteFornecedorResourceIT {
    ...
    private static final String DEFAULT_UF_HARAS = "AAAAAAAAAA";
    private static final String UPDATED_UF_HARAS = "BBBBBBBBBB";
     ...
     *
     * @see br.com.mmgestor.repository.search.ClienteFornecedorSearchRepositoryMockConfiguration
     */
    @Autowired
    private ClienteFornecedorSearchRepository mockClienteFornecedorSearchRepository;
    ...
    public void createClienteFornecedor() throws Exception {
        ...
        // Validate the ClienteFornecedor in Elasticsearch
        verify(mockClienteFornecedorSearchRepository, times(1)).save(testClienteFornecedor);
    }
    ...
    public void createClienteFornecedorWithExistingId() throws Exception {
        ...
        // Validate the ClienteFornecedor in Elasticsearch
        verify(mockClienteFornecedorSearchRepository, times(0)).save(clienteFornecedor);
    }
    ...
    public void updateClienteFornecedor() throws Exception {
        ...
        reset(mockClienteFornecedorSearchRepository);
    ...
    public void deleteClienteFornecedor() throws Exception {
        ...
        // Validate the ClienteFornecedor in Elasticsearch
        verify(mockClienteFornecedorSearchRepository, times(1)).deleteById(clienteFornecedor.getId());
    }

    @Test
    @Transactional
    public void searchClienteFornecedor() throws Exception {
        // Initialize the database
        clienteFornecedorService.save(clienteFornecedor);
        when(mockClienteFornecedorSearchRepository.search(queryStringQuery("id:" + clienteFornecedor.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(clienteFornecedor), PageRequest.of(0, 1), 1));
        // Search the clienteFornecedor
        restClienteFornecedorMockMvc.perform(get("/api/_search/cliente-fornecedors?query=id:" + clienteFornecedor.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(clienteFornecedor.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)))
