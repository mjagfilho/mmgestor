...
public class DadosAssociacaoResourceIT {
     ...
     *
     * @see br.com.mmgestor.repository.search.DadosAssociacaoSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createDadosAssociacao() throws Exception {
        ...
        // Validate the DadosAssociacao in Elasticsearch
        verify(mockDadosAssociacaoSearchRepository, times(1)).save(testDadosAssociacao);
    }
    ...
    public void createDadosAssociacaoWithExistingId() throws Exception {
        ...
        // Validate the DadosAssociacao in Elasticsearch
        verify(mockDadosAssociacaoSearchRepository, times(0)).save(dadosAssociacao);
    }
    ...
    public void updateDadosAssociacao() throws Exception {
        ...
        reset(mockDadosAssociacaoSearchRepository);
    ...
    public void deleteDadosAssociacao() throws Exception {
        ...
        // Validate the DadosAssociacao in Elasticsearch
        verify(mockDadosAssociacaoSearchRepository, times(1)).deleteById(dadosAssociacao.getId());
    }

    @Test
    @Transactional
    public void searchDadosAssociacao() throws Exception {
        // Initialize the database
        dadosAssociacaoService.save(dadosAssociacao);
        when(mockDadosAssociacaoSearchRepository.search(queryStringQuery("id:" + dadosAssociacao.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(dadosAssociacao), PageRequest.of(0, 1), 1));
        // Search the dadosAssociacao
        restDadosAssociacaoMockMvc.perform(get("/api/_search/dados-associacaos?query=id:" + dadosAssociacao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(dadosAssociacao.getId().intValue())))
            .andExpect(jsonPath("$.[*].criador").value(hasItem(DEFAULT_CRIADOR)))
