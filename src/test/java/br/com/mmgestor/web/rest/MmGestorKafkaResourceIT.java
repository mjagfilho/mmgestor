...
import br.com.mmgestor.config.KafkaProperties;
...
class MmGestorKafkaResourceIT {
    ...
    void setup() {
        ...
        MmGestorKafkaResource kafkaResource = new MmGestorKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mm-gestor-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mm-gestor-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
