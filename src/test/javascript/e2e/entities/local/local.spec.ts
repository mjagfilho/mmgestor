...
describe('Local e2e test', () => {
  ...
  it('should load Locals', async () => {
    ...
    expect(await localComponentsPage.getTitle()).to.eq('mmGestorApp.local.home.title');
  });

  it('should load create Local page', async () => {
    await localComponentsPage.clickOnCreateButton();
    localUpdatePage = new LocalUpdatePage();
    expect(await localUpdatePage.getPageTitle()).to.eq('mmGestorApp.local.home.createOrEditLabel');
  ...
  /*  it('should create and save Locals', async () => {
        ...
        await localComponentsPage.clickOnCreateButton();
        await promise.all([
            localUpdatePage.setNomeInput('nome'),
            localUpdatePage.setAreaInput('5'),
            localUpdatePage.tipoSelectLastOption(),
            localUpdatePage.enderecoSelectLastOption(),
            localUpdatePage.paiSelectLastOption(),
        ]);
        expect(await localUpdatePage.getNomeInput()).to.eq('nome', 'Expected Nome value to be equals to nome');
        expect(await localUpdatePage.getAreaInput()).to.eq('5', 'Expected area value to be equals to 5');
        const selectedEhContigua = localUpdatePage.getEhContiguaInput();
        if (await selectedEhContigua.isSelected()) {
            await localUpdatePage.getEhContiguaInput().click();
            expect(await localUpdatePage.getEhContiguaInput().isSelected(), 'Expected ehContigua not to be selected').to.be.false;
        } else {
            await localUpdatePage.getEhContiguaInput().click();
            expect(await localUpdatePage.getEhContiguaInput().isSelected(), 'Expected ehContigua to be selected').to.be.true;
        }
  ...
  /*  it('should delete last Local', async () => {
        ...
        expect(await localDeleteDialog.getDialogTitle())
            .to.eq('mmGestorApp.local.delete.question');
