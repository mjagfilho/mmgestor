...
public class ClienteFornecedorResource {
    ...
    /**
     * {@code SEARCH  /_search/cliente-fornecedors?query=:query} : search for the clienteFornecedor corresponding
     * to the query.
     *
     * @param query the query of the clienteFornecedor search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/cliente-fornecedors")
    public ResponseEntity<List<ClienteFornecedor>> searchClienteFornecedors(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ClienteFornecedors for query {}", query);
        Page<ClienteFornecedor> page = clienteFornecedorService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
