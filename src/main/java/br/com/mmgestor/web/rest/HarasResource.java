...
public class HarasResource {
    ...
    /**
     * {@code SEARCH  /_search/haras?query=:query} : search for the haras corresponding
     * to the query.
     *
     * @param query the query of the haras search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/haras")
    public ResponseEntity<List<Haras>> searchHaras(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Haras for query {}", query);
        Page<Haras> page = harasService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
