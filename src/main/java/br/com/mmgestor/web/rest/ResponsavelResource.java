...
public class ResponsavelResource {
    ...
    public ResponseEntity<List<Responsavel>> searchResponsavels(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Responsavels for query {}", query);
        Page<Responsavel> page = responsavelService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
